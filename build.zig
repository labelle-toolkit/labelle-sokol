...
fn addBackendGraph(
    ...
    if (is_android) {
        ...
        // The shared package compiles its own JNI C against its NDK sysroot.
        // Keep the gamepad bridge above independent; only the duplicated
        // launch-intent/debuggable services move here (#27).
        const android_dep = b.dependency("labelle_android", .{ .target = target, .optimize = optimize });
        input_mod.addImport("labelle_android", android_dep.module("labelle_android"));
    }
